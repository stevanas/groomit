...
- **Browse filters**: category chips fixed — "Both/Και τα 2" now returns ONLY combo (groomer+shop) stores, distinct from "All". Results **paginate** via Google `next_page_token` (infinite scroll, `/api/places/nearby?page_token=`). Reviews are limited to ~5 by Google; shop details shows a "See all reviews on Google" deep link. Home "στην περιοχή" field uses Google Places Autocomplete (`/api/places/autocomplete`, region-biased to GR) with a dropdown + "Use my location" option; selecting a place navigates to correctly-filtered nearby stores.
- **Maps**: live GPS device location (expo-location) powers nearby results & distance sort. Decluttered Google map (POI/transit/business labels hidden). Cohesive Ionicons pins (cut / storefront / ribbon, white on colored circle); focused store enlarged. Maps re-center on resolved location (animateToRegion). Map preview card on Home ("Κοντά σου") and a Location section on shop details (store highlighted + nearby, tighter zoom); both expand to a full-screen `/map` route. Tapping a pin on the full map shows a bottom info card (photo, rating, category, open status) → tap to open details. Maps/GPS render only in a native build, not the web preview.
- **Theming**: full **light + dark mode** via `ThemeProvider`/`useTheme` (palettes in `theme.ts`, runtime styles via `useThemedStyles(makeStyles)`). First-launch prompt (`ThemePrompt`) lets users choose Light/Dark; changeable anytime in Profile → Appearance (persisted, key `theme_mode`). StatusBar adapts to mode.
- **Monetization (native build only)**: discreet AdMob banner (`react-native-google-mobile-ads`) at the bottom of the results list, hidden for premium users. AdMob App ID set in app.json (`ca-app-pub-9770198187060268~96865297343`); uses Google TEST ad unit IDs in dev — **TODO: replace `REAL_BANNER_UNIT_ID` in `src/components/AdBanner.tsx` with a real banner unit ID, and set the real iOS App ID in app.json before release**. In-app purchase via `expo-iap` (no RevenueCat): single non-consumable `remove_ads_lifetime` that permanently removes ads + supports the dev. Premium flag stored on-device (`src/premium.tsx`, AsyncStorage key `premium_remove_ads`); reconciled with the store + restore-purchases. "Remove ads & Support" card lives in the Profile → Support section. **TODO: create the `remove_ads_lifetime` non-consumable product in App Store Connect & Google Play.** Ads & purchases only work in a published native build, NOT Expo Go/web (all native calls are guarded so the preview still loads).
